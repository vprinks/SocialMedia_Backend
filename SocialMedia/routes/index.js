    ...
    async function (req, res, next) {
        try {
            ...
            if (error.name === 'ValidationError') {
                res.render('postcreate', { user: req.user, errors: error.errors, title: req.body.title });
            } else {
...
//         if (deletepost.profilepic !== "default.png"){
//         fs.unlinkSync(
//             path.join(__dirname, "..", "public", "images", deletepost.profilepic)
//         );
...
router.get("/delete-post/:id", isLoggedIn, async function (req, res, next) {
    try {
        const deletepost = await Post.findByIdAndDelete(req.params.id);
        if (deletepost && deletepost.profilepic !== "default.png") {
            fs.unlinkSync(
                path.join(__dirname, "..", "public", "images", deletepost.profilepic)
            );
        }
